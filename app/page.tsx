...
export default function Home() {
  ...
  const [authPromptState, setAuthPromptState] = useState<{ isOpen: boolean; jobId: number | null }>({
    isOpen: false,
    jobId: null,
  });
  ...
  return (
    <>
      ...
      <main className="flex-grow flex flex-col w-full max-w-[1200px] mx-auto px-6 py-12 gap-24">
        ...
        <section className="flex flex-col md:flex-row items-center gap-12 mt-8">
          <div className="flex-1 flex flex-col gap-6">
            ...
            <h1 className="font-display-lg text-display-lg text-zinc-900 max-w-[42rem] font-bold">
              Finding a job you love shouldn&apos;t be <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">this hard.</span>
            </h1>
            <p className="text-lg text-zinc-500 max-w-[36rem] leading-relaxed">
              We&apos;re tired of endless forms and black-hole applications. TalentBridge is a simpler way to connect with teams building the future, right here in India. No password required.
            </p>
            <div className="flex flex-wrap items-center gap-4 mt-2">
              <button 
                ...
                className="bg-blue-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-600/20 active:scale-95 duration-150"
              >
                Find Jobs
              </button>
              <Link href="/employers">
                <button className="bg-blue-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-blue-700 transition-all flex items-center gap-2 active:scale-95 duration-150 animate-pulse-soft">
          ...
          <div className="flex-1 relative w-full aspect-square max-w-[500px]">
            <div className="absolute inset-0 bg-gradient-to-tr from-blue-50 to-indigo-50 opacity-50 rounded-2xl"></div>
            <img 
              alt="Professionals working" 
              className="absolute inset-4 object-cover rounded-xl shadow-2xl border border-zinc-100 w-[calc(100%-2rem)] h-[calc(100%-2rem)]" 
        ...
        <section className="w-full py-12 relative overflow-hidden bg-white border-y border-zinc-200 rounded-3xl my-8">
          ...
          <p className="text-center text-zinc-400 mb-8 uppercase tracking-[0.3em] text-[11px] font-semibold">
            ...
            <div className="flex items-center gap-20 md:gap-32 animate-scroll pause-on-hover px-4">
              {[...Array(2)].map((_, i) => (
                <React.Fragment key={i}>
                  ...
                  <img src="https://upload.wikimedia.org/wikipedia/commons/0/00/Anthropic_logo.svg" alt="Anthropic" className="h-6 w-auto object-contain hover:scale-110 transition-all duration-300" />
        ...
        {/* Featured Companies Section (Accel Style) */}
        <section id="featured-companies" className="flex flex-col gap-12 scroll-mt-24 py-16">
          <div className="flex flex-col md:flex-row md:items-end justify-between px-6">
            <div className="flex-1">
              <h2 className="text-3xl font-bold text-zinc-900">Featured Companies</h2>
          ...
          {/* Horizontal Gallery with Auto-scroll */}
          <div className="w-full overflow-hidden relative group">
            <div 
              ref={scrollRef}
              className="flex gap-6 px-6 overflow-x-auto no-scrollbar scroll-smooth pb-8"
            >
              ...
              {[...COMPANIES, ...COMPANIES].map((company, index) => (
                <Link 
                  ...
                  className="min-w-[300px] md:min-w-[380px] aspect-[4/5] relative rounded-[40px] overflow-hidden group/card shadow-xl hover:shadow-2xl transition-all duration-700 border border-zinc-200"
                >
                  <img 
                    src={company.bgImage} 
                    alt={company.name} 
                    className="absolute inset-0 w-full h-full object-cover group-hover/card:scale-105 transition-transform duration-1000 grayscale-[0.2] group-hover/card:grayscale-0" 
                  />
                  {/* Subtle vignette for CEO portrait feel */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent"></div>
                  
                  <div className="absolute top-8 right-8 bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/20">
                    <img src={company.logo} alt={company.name} className="h-6 w-auto brightness-0 invert" />
                  </div>

                  <div className="absolute bottom-0 left-0 p-10 w-full">
                    <div className="mb-4 inline-flex items-center gap-2 px-3 py-1 bg-blue-600/20 backdrop-blur-sm border border-blue-400/30 rounded-full">
                      <div className="w-2 h-2 bg-blue-400 rounded-full animate-pulse"></div>
                      <span className="text-[10px] font-bold text-blue-100 uppercase tracking-widest">Hiring Directly</span>
                    </div>
                    <h3 className="text-3xl font-bold text-white mb-2 leading-tight tracking-tight">{company.name}</h3>
          ...
          {/* Logo Grid */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-6 mt-8">
            {COMPANIES.map((company) => (
              <Link 
                key={company.id} 
                href={`/companies/${company.id}`}
                className="bg-zinc-100/50 hover:bg-white border border-transparent hover:border-zinc-200 rounded-2xl p-8 flex items-center justify-center transition-all duration-300 group/logo shadow-sm hover:shadow-md h-32"
              >
                <img 
                  src={company.logo} 
                  alt={company.name} 
                  className="max-h-10 w-auto object-contain grayscale group-hover/logo:grayscale-0 transition-all duration-500" 
                />
              </Link>
            ))}
          </div>
        ...
        <section id="browse-jobs" className="flex flex-col gap-12 scroll-mt-24 py-16 px-8 bg-white rounded-[40px] border border-zinc-100 my-8">
          <div className="flex flex-col gap-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div className="flex-1">
                <h2 className="text-3xl font-bold text-zinc-900">Current Openings</h2>
                <p className="text-zinc-500 mt-3 max-w-2xl leading-relaxed">We focus on quality over quantity. Here are the roles currently open at our partner companies across India.</p>
              </div>
            </div>
            ...
            {displayedJobs.length === 0 ? (
              ...
              displayedJobs.map((job) => (
                <div 
                  ...
                  className="group bg-white hover:bg-zinc-50/30 p-6 rounded-[24px] border border-zinc-100 hover:border-blue-600/20 hover:shadow-2xl hover:shadow-blue-600/5 transition-all duration-300 h-full flex flex-col cursor-pointer"
                >
                  ...
                  <div className="mt-8 pt-6 border-t border-zinc-100 flex items-center justify-between">
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setAuthPromptState({ isOpen: true, jobId: job.id });
                      }}
                      className="bg-white border border-zinc-200 text-zinc-600 font-bold px-4 py-2 rounded-xl group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 transition-all text-xs"
                    >
                      Apply Now
                    </button>
                  </div>
        ...
        <section id="about" className="flex flex-col gap-8 scroll-mt-24 py-12">
          <div className="bg-white border border-zinc-200 rounded-[32px] p-8 md:p-12 shadow-sm relative overflow-hidden">
            ...
            <h2 className="text-3xl font-bold text-zinc-900 mb-8 relative z-10">Why we built this</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center relative z-10">
              <div className="flex flex-col gap-6 text-zinc-500">
                <p className="leading-relaxed">
                  Let&apos;s be honest: <strong className="text-zinc-900">most job boards are broken.</strong> You spend hours on a profile just to never hear back.
                </p>
                <p className="leading-relaxed">
                  We built TalentBridge because we wanted a place where your work speaks louder than your resume. We don&apos;t want you to waste time setting up accounts or remembering passwords.
                </p>
                <p className="leading-relaxed">
                  We focus on India&apos;s tech scene because we believe some of the most exciting work in the world is happening right here in Bangalore, Mumbai, and Hyderabad.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-6">
                <div className="bg-zinc-50/80 p-7 rounded-2xl border border-zinc-100 hover:border-blue-500/30 hover:bg-white transition-all duration-300 shadow-sm">
                  <h3 className="text-lg font-bold text-zinc-900 mb-3 flex items-center gap-3">
                    <span className="material-symbols-outlined text-blue-600">verified</span>
                    Built for fairness
                  </h3>
                  <p className="text-sm text-zinc-500 leading-relaxed">
                    We try to remove bias where we can. We hide names and photos during the first round of review so hiring managers focus on what you can actually do.
                  </p>
                </div>
                ...
                <div className="bg-zinc-50/80 p-7 rounded-2xl border border-zinc-100 hover:border-blue-500/30 hover:bg-white transition-all duration-300 shadow-sm">
                  ...
                  <p className="text-sm text-zinc-500 leading-relaxed">
                    Apply in about 90 seconds. No "create account" roadblocks. If you like a role, just hit apply and show them what you&apos;ve got.
                  </p>
                </div>
              </div>
            </div>
      ...
      <footer className="bg-white font-inter text-xs tracking-wide border-t border-zinc-200 w-full mt-auto">
        <div className="flex flex-col md:flex-row justify-between items-center px-8 py-12 max-w-7xl mx-auto w-full gap-4">
          <div className="text-lg font-black text-zinc-900">TalentBridge</div>
          <div className="text-zinc-500">© 2024 TalentBridge. The Modern Way to Hire Top Talent.</div>
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a className="text-zinc-500 hover:text-zinc-900 hover:underline transition-all opacity-80 hover:opacity-100" href="#">Privacy Policy</a>
            <a className="text-zinc-500 hover:text-zinc-900 hover:underline transition-all opacity-80 hover:opacity-100" href="#">Terms of Service</a>
            <a className="text-zinc-500 hover:text-zinc-900 hover:underline transition-all opacity-80 hover:opacity-100" href="#">Cookie Policy</a>
            <a className="text-zinc-500 hover:text-zinc-900 hover:underline transition-all opacity-80 hover:opacity-100" href="#">Accessibility</a>
            <a className="text-zinc-500 hover:text-zinc-900 hover:underline transition-all opacity-80 hover:opacity-100" href="#">API Status</a>
          </div>
        </div>
      </footer>
